...
public sealed class VersionTester
{
	...
	{
		OVSXmlSerializer<StandardClass> xmlSerializer = new(new() { Version = new Version(2, 4) });
	...
	{
		OVSXmlSerializer<StandardClass> xmlSerializer = new(new() { Version = new Version(2, 5) });
		using var memoryStream = xmlSerializer.Serialize(new StandardClass());
	...
	{
		OVSXmlSerializer<StandardClass> xmlSerializer = new(new() { Version = new Version(2, 5), VersionLeniency = Versioning.Leniency.Minor });
		using var memoryStream = xmlSerializer.Serialize(new StandardClass());
	...
	{
		OVSXmlSerializer<StandardClass> xmlSerializer = new(new() { Version = new Version(3, 4), VersionLeniency = Versioning.Leniency.Minor });
		using var memoryStream = xmlSerializer.Serialize(new StandardClass());
